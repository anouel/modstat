...
- On peut montrer que $S^2=\mathsf{SS}_e/(n-p-1)$ est un estimateur non-biaisé de la variance $\sigma^2$.
- Aussi, $\mathsf{SS}_e \sim \sigma^2 \chi^2_{n-p-1}$ et $\mathsf{SS}_e$ est indépendant de $\widehat{\boldsymbol{\beta}}$.
...
\end{align*}
où $S^2=\mathsf{SS}_e/(n-p-1)$ est l'estimateur sans biais de la variance $\sigma^2$.
...
\end{align*}
où $\hat{Y}_i^{\mathbb{M}_1}$ est la  $i$e valeur ajustée du  modèle $\mathbb{M}_1$. On définit de la même façon la somme du carré des résidus,  $\mathsf{SS}_e(\mathbb{M}_0)$, pour le modèle $\mathbb{M}_0$.
...
Soit $\mu_{0}, \mu_{0.5}, \mu_{1}, \mu_2$ la vraie moyenne du score PEF en fonction de la proportion de papier pour les données de @Sokolova:2023.

 Nous pouvons comparer le modèle de régression linéaire (dans lequel le score PEF augmente linéairement avec la proportion de papier par rapport au plastique),
  \begin{align*}
...
- la prédiction des moyennes de groupe pour une valeur commune des autres variables explicatives.
 ...
 La variance du contraste avec des sous-échantillons de tailel $n_1, \ldots, n_K$ et une variance commune $\sigma^2$, est
\begin{align*}
...
es auteurs souhaitaient comparer zéro carton avec d'autres choix: nous nous intéressons aux différences par paire, mais uniquement par rapport à la référence $\mu_{0}$:
\begin{align*}
