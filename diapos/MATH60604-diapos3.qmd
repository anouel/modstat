...
- $\boldsymbol{\beta} = (\beta_0, \ldots, \beta_p)^\top$ est un vecteur colonne de longeur $p+1$ contenant les coefficients de la moyenne.
...
- de l'estimateur des moindres carrés ordinaires est $\mathsf{Var}(\widehat{\boldsymbol{\beta}})=\sigma^2(\mathbf{X}^\top\mathbf{X})^{-1}$ et
- de l'EMV de la variance $\mathsf{Var}(\widehat{\sigma}^2)=2\sigma^4/n$.
