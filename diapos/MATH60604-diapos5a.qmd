...
$$\underset{\text{total}}{\mathsf{SC}_{c}} = \underset{\text{facteu $A$}}{\mathsf{SC}_A} + \underset{\text{facteur $B$}}{\mathsf{SC}_B} + \underset{\text{interaction}}{\mathsf{SC}_{AB}} + \underset{\text{résidus}}{\mathsf{SC}_e}$$
Ainsi, 
...
Si le modèle alternatif a $n_an_b$ paramètres pour la moyenne, et que nous imposons $n_a-1$ restrictions linéaires sous l'hypothèse nulle au modèle estimé sur la base de $n$ observations indépendantes, la statistique de test $F$ est
\begin{align*}
...
means <- model.tables(mod1, type = "means")
```
...
La statistique $F$ vaut `r round(car::Anova(mod, type = 3)[4,3],2)`; avec la loi nulle $\mathsf{Fisher}$ (`r car::Anova(mod, type = 3)[4,2]`, `r car::Anova(mod, type = 3)[5,2]`), la valeur-$p$ est `r format.pval(car::Anova(mod, type = 3)[4,4], digits=2)`. Il n'y a pas d'effet du type d'achat sur la dette.
...
## Tableau d'analyse de variance pour l'exemple 2 
