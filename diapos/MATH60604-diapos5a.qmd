...
Considérons le retrait de $A$ du modèle balancé: si le modèle alternatif a $n_an_b$ paramètres pour la moyenne, et que nous imposons $n_a-1$ restrictions linéaires sous l'hypothèse nulle au modèle estimé sur la base de $n$ observations indépendantes, la statistique de test $F$ est
\begin{align*}
