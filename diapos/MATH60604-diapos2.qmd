...
g1 <- ggplot(data = data.frame(time = attente),
  ...
  geom_rug() +
  labs(x = "attente time (in seconds)")
...
Dénotons la vraie valeur du vecteur de paramètres $\boldsymbol{\theta}_0.$ Sous des conditions de régularité appropriées, le théorème central limite donne
\begin{align*}
...
En pratique, la véritable valeur du paramètre $\boldsymbol{\theta}_0$ étant inconnue, nous évaluons l'information aux EMV $\widehat{\boldsymbol{\theta}}$. Cela se justifie par le fait que les matrices d'information de Fisher et observées, $i(\widehat{\boldsymbol{\theta}})$ et $j(\widehat{\boldsymbol{\theta}})$, convergent vers $i(\boldsymbol{\theta}_0)$ quand $n \to \infty$.]
...
Si $\phi = g(\boldsymbol{\theta})$, où $g: \mathbb{R}^p \to \mathbb{R}^k$ pourr $k \leq p$ est une fonction différentiable de $\boldsymbol{\theta}$ non-nulle à $\boldsymbol{\theta}_0$ alors
...
#| fig-cap: "Vraisemblance profilée pour $\\alpha$, représentée par une ligne grise pointillée (à gauche) et par une coupe transversale (à droite). La vraisemblance profilée du le panneau de droite a été décalée verticalement pour être zéro quand évaluée aux EMV; les lignes horizontales en pointillé indiquent les points de coupure pour les intervalles de confiance à 95% et 99%."
#| fig-align: 'center'
#| eval: true
#| echo: false
# Profile likelihood for alpha
