...
modlin.college1 <- lm(salaire ~ echelon + domaine + sexe + annees + service, data = college)
car::residualPlots(modlin.college1, test = FALSE, layout = c(2,3))
```
...
Les choix populaires pour des matrices convergente en cas d'hétéroscédasticité matrices [@McKinnon.White:1985], utilisent $\mathrm{diag}(\boldsymbol{\Omega})_i = e_i^2/(1-h_{ii})^2$, dans le cas de la matrice HC${}_3$.
...
Avec des données strictement positives, on peut utiliser une transformation de Box--Cox,
\begin{align*}
